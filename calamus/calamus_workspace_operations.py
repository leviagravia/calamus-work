...
def normalize_workspace_basename(raw_name: str, *, suffix: str = DEFAULT_TEXT_SUFFIX) -> str:
    ...
    name = raw_name.strip()
    if not name:
        raise WorkspaceOperationError("Enter a file name.")
    if "\x00" in name:
        raise WorkspaceOperationError("The file name contains an invalid character.")
    separators = {os.sep, "/", "\\"}
    if os.altsep:
        separators.add(os.altsep)
    if any(separator and separator in name for separator in separators):
        raise WorkspaceOperationError("Enter one file name, not a path.")
    if name in {".", ".."} or name.startswith("."):
        raise WorkspaceOperationError("Hidden or reserved file names are not allowed here.")

...
TMP_CACHE="$(mktemp -d)"
LOG="${CALAMUS_W92_HEADLESS_LOG:-$(mktemp)}"
...
cat "$LOG"
[[ "$status" -eq 0 ]] || {
    echo "W92_HEADLESS_REGRESSION=FAIL status=$status"
    exit 1
}
count="$(sed -nE 's/^Ran ([0-9]+) tests.*/\1/p' "$LOG" | tail -n 1)"
[[ -n "$count" ]] || {
    echo "W92_HEADLESS_REGRESSION=FAIL missing-count"
    exit 1
}
[[ "$count" -eq 1363 ]] || {
    echo "W92_HEADLESS_REGRESSION=FAIL count=$count"
    exit 1
}
for token in \
    ...
    if grep -Fq "$token" "$LOG"; then
        echo "W92_HEADLESS_REGRESSION=FAIL diagnostic=$token"
        exit 1
    fi
done
echo "W92_HEADLESS_REGRESSION_COUNT=$count"
echo "W92_HEADLESS_REGRESSION=PASS"
echo "W92_HEADLESS_GTK_WORKFLOWS_ISOLATED=PASS"

...
class QuitCommandWiringTests(unittest.TestCase):
    ...
    def test_gateway_orders_decision_settings_and_destroy(self):
        ...
        geometry_at = method.index("self.record_window_geometry()")
        last_file_at = method.index("self.record_last_file()")
        flag_at = method.index('self._application_close_in_progress = True')
        destroy_at = method.index("self.destroy()")
        self.assertLess(prompt_at, geometry_at)
        self.assertLess(geometry_at, last_file_at)
        self.assertLess(last_file_at, flag_at)

...
class LowercaseLayerWiringTests(unittest.TestCase):
    ...
    def test_dispatch_surface_is_unchanged_because_lowercase_id_already_existed(self):
        ...
        self.assertEqual(_dispatch_ids().count("edit.lowercase"), 1)
    ...
    def test_replace_selection_preserves_selection_only_policy(self):
        ...
        dispatch_pos = method.index("self.command_layer_lowercase_text(txt)")
    ...
    def test_replace_selection_routes_lowercase_before_plan_and_mutation(self):
        method = _app_methods()["replace_selection"]
        self.assertIn('command_name="Replace Selection"', method)
        self.assertIn("use_layer_lowercase", method)
        self.assertIn(
            'command_name in {"Lowercase", "Lower Case", "LOWERCASE"}',
            method,
        )
        self.assertIn("transform is str.lower", method)
        self.assertIn("replacement, changed = self.command_layer_lowercase_text(txt)", method)
    ...
    def test_uppercase_branch_remains_distinct_and_precedes_lowercase(self):
        method = _app_methods()["replace_selection"]
        upper_branch = method.index("if use_layer_uppercase:")
        upper_dispatch = method.index("self.command_layer_uppercase_text(txt)")
        lower_branch = method.index("elif use_layer_lowercase:")
        lower_dispatch = method.index("self.command_layer_lowercase_text(txt)")
        self.assertLess(upper_branch, upper_dispatch)
        self.assertLess(upper_dispatch, lower_branch)
        self.assertLess(lower_branch, lower_dispatch)

    def test_lowercase_noop_guard_precedes_plan_and_mutation(self):
        method = _app_methods()["replace_selection"]
        branch_pos = method.index("elif use_layer_lowercase:")
    ...
    def test_visible_entrypoint_uses_selection_boundary_and_command_name(self):
        method = _app_methods()["on_lowercase"]
        self.assertEqual(
            method.strip(),
    ...
    def test_existing_selection_or_document_lowercase_bridge_remains_available(self):
        method = _app_methods()["apply_text_transform"]
        self.assertIn("use_layer_lowercase", method)
        self.assertIn("command_layer_lowercase_text(old)", method)
        self.assertIn("transform is str.lower", method)
        self.assertIn(
            'command_name in {"Lowercase", "Lower Case", "LOWERCASE"}',
            method,

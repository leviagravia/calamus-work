...
def load_app():
    ...
    name = "w106_identity_" + uuid.uuid4().hex
...
@unittest.skipUnless(RUN and HAVE_GTK and display_ready(), "real W106 GTK identity lane")
class W106IdentityRealAppE2E(unittest.TestCase):
                ...
                try:
                    ...
                    def close_system_info():
                        ...
                        for token in (
                            ...
                            "Work item: W106",
                            "Work item description: Preferences and Application State Extraction",
                            "Published baseline: aa73cc830b2c2120e26fd7ffb5d21b56c95e709b",
                        ):
                            self.assertIn(token, body)
                        dialog.response(Gtk.ResponseType.CLOSE)
                        return True
                    driver = ModalDriver([close_system_info]); driver.start(); window.on_system_info(); pump(); driver.assert_complete()
                    print("W106_CURRENT_IDENTITY=PASS")
                finally:
                    window.destroy(); pump()

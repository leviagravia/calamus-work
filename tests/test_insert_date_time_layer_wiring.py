...
class InsertDateTimeLayerWiringTests(unittest.TestCase):
    ...
    def test_entrypoints_use_shared_insert_boundary_with_explicit_formats(self):
        ...
        self.assertIn("ports.command_layer_insert_date_time_text(ports.now(), fmt)", bridge)
        self.assertIn("command_insert_at(ports.buffer_text(), cursor, txt)", bridge)
        self.assertIn("buf.insert_at_cursor(txt)", bridge)
        self.assertIn("ports.execute_command(command_name, edit, select_range=select)", bridge)
        self.assertIn('insert_current_moment(ports, "Insert Date", "%Y-%m-%d")', bridge)
        self.assertIn('insert_current_moment(ports, "Insert Time", "%H:%M")', bridge)
        self.assertIn('insert_current_moment(ports, "Insert Date and Time", "%Y-%m-%d %H:%M")', bridge)
        self.assertIn("WritingInsertPorts(", launcher)
        self.assertIn("on_insert_datetime=partial(writing_app.on_insert_datetime, writing_insert_ports)", launcher)

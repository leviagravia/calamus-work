...
class W100MonolithDecompositionContractTests(unittest.TestCase):
    ...
    def test_baseline_metrics_are_exact_and_growth_is_blocked(self):
        ...
        self.assertEqual(len(self.launcher.splitlines()), 3298)
        self.assertEqual(self.app.end_lineno - self.app.lineno + 1, 3066)
        self.assertEqual(len(self.methods), 266)
    ...
    def test_every_app_method_is_assigned_once(self):
        ...
        self.assertEqual(set(names), {m.name for m in self.methods})
    ...
    def test_whole_app_coupling_inventory_is_exact(self):
        ...
        for path in sorted((ROOT / "calamus").glob("*.py")):
            ...
            for node in ast.walk(tree):
                ...
                if "app" in args:
                    expected.add((path.relative_to(ROOT).as_posix(), node.name, node.lineno))
        data = self.load("CALAMUS_W100_WHOLE_APP_COUPLING_INVENTORY.json")
        actual = {(r["path"], r["function"], r["start_line"]) for r in data["couplings"]}
        self.assertEqual(actual, expected)
        self.assertTrue(actual)
        self.assertTrue(all(r["migration_work_item"] in {"W105", "W107"} for r in data["couplings"]))
    ...
    def test_no_product_or_launcher_behavior_was_changed_by_w100(self):
        # W100 changes identity/docs/tests/release gates only. The launcher remains the W99 source.
        metrics = self.load("CALAMUS_W100_BASELINE_METRICS.json")
        self.assertEqual(metrics["launcher_lines"], 3298)
        self.assertNotIn("ApplicationComposer(", self.launcher)

...
class W107SubsystemHostPortContractTests(unittest.TestCase):
    def test_identity_is_exact_and_w108_is_not_started(self):
        ...
        self.assertNotIn('DEVELOPMENT_WORK_ITEM = "W108"', version)
    ...
    def test_recent_workspace_projection_waits_for_core_composition_barrier(self):
        menu_builder = class_method_source("bin/calamus", "App", "__init__")
        ui_builder = source("calamus/calamus_ui.py")
        self.assertNotIn("app.populate_recent_workspaces_menu()", ui_builder)
        composition = menu_builder.index("self._components = compose_core_application_components(")
        projection = menu_builder.index("self.populate_recent_workspaces_menu()")
        self.assertGreater(projection, composition)
        self.assertIn(
            "self._components.workspace.host_runtime.populate_recent_workspaces_menu()",
            class_method_source("bin/calamus", "App", "populate_recent_workspaces_menu"),
        )
    ...
    def test_search_spell_print_use_one_private_typed_bundle(self):
        ...
        self.assertIn("self._w107_subsystems.search.on_find_replace", app)
        self.assertIn("self._w107_subsystems.spellcheck.on_check", app)
        self.assertIn("self._w107_subsystems.printer.on_print", app)

    def test_app_w107_methods_are_compatibility_delegates_not_domain_owners(self):
        ...
        present = [methods[name] for name in names if name in methods]
        self.assertEqual(len(present), 116)
        # All command compatibility methods except the static Research alias projection
        # and three pre-construction-safe invalidation gates are bounded shell code.
        long = {n.name: n.end_lineno - n.lineno + 1 for n in present if n.end_lineno - n.lineno + 1 > 6}

...
from calamus_version import (
    DEVELOPMENT_BUILD_LABEL, DEVELOPMENT_WORK_ITEM,
    DEVELOPMENT_WORK_ITEM_DESCRIPTION, PUBLISHED_BASELINE,
)
...
EXPECTED = (
    ...
    "W108",
    "Thin GTK Shell",
    "e16cc21b8a900298406ae8cc4776f6f1ec658e93",
)
...
def load_app():
    ...
    name = "w108_identity_" + uuid.uuid4().hex
...
def text_view_body(view):
    buffer = view.get_buffer(); start, end = buffer.get_bounds()
    return buffer.get_text(start, end, True)


@unittest.skipUnless(RUN and HAVE_GTK and display_ready(), "real W108 GTK identity lane")
class W108IdentityRealAppE2E(unittest.TestCase):
                ...
                try:
                    ...
                    def close_system_info():
                        ...
                        for token in (
                            ...
                            "Work item: W108",
                            "Work item description: Thin GTK Shell",
                            "Published baseline: e16cc21b8a900298406ae8cc4776f6f1ec658e93",
                        ):
                            self.assertIn(token, body)
                        dialog.response(Gtk.ResponseType.CLOSE)
                        return True
                    driver = ModalDriver([close_system_info]); driver.start(); window.on_system_info(); pump(); driver.assert_complete()
                    print("W108_CURRENT_IDENTITY=PASS")
                finally:
                    window.destroy(); pump()
